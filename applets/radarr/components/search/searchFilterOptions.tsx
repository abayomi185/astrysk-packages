...
const createSettingsOptionsObject = (
  ...
  filterType: RadarrFilterKind,
  selectedValue: RadarrFilterKindValue,
  first?: boolean,
  last?: boolean
) => {
  return {
...
const RadarrSearchFilterOptions: React.FC<{
  context: RadarrSearchFilterContext;
  filterType: string;
}> = ({ context, filterType }) => {
  ...
  const selectedValue =
    useRadarrStore((state) => state?.searchFilters?.[context]?.[filterType]) ??
    {};

  const filterBarOptions = useRadarrStore.getState().filterBarOptions;
...
export default RadarrSearchFilterOptions;
