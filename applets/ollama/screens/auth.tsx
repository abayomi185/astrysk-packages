...
import { configureAxiosForOllama } from "../utils";
...
const OllamaAuth = () => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<OllamaAuthInputs>();

  const [apiToken, setApikey] = React.useState<string>();
  ...
  const onSubmit: SubmitHandler<OllamaAuthInputs> = (
    data: OllamaAuthInputs
  ) => {
    ...
    const serverURL = partialServerURL;

    useOllamaStore.setState({
      baseURL: serverURL,
      tokenId: data.token,
    });
    configureAxiosForOllama(serverURL, data.token, undefined, () => {
  ...
  return (
    <YStack height="100%">
      <KeyboardAwareScrollView keyboardShouldPersistTaps="handled">
        <AppletButtonBanner applet={Applets.OLLAMA} />
        <Form
          space
          width="100%"
          paddingHorizontal="$4"
